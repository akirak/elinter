...
Even though this program is named after MELPA, it is not written by authors of the service.
** Table of contents
:PROPERTIES:
:TOC:      siblings 
:END:
...
- Testing multiple packages in a single project/repository: With local dependencies, it allow you to run =package-lint= and other tasks even if your package depends on another package that is not available on MELPA yet.
...
- Command line interface for easily running linting tasks both locally use on CI.
...
- =all= :: All available versions including =minimum= and later.
