...
The recommended file name is =default.nix=, but it can be any file name with =.nix= suffix.
...
If the name of the configuration file is =default.nix=, you can check the package as follows:
...
By default, an Emacs package specified after =?= in the input argument is used to build the Emacs package.
You can override this value from the command line.
