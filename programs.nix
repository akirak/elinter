{ emacs, srcDir, packageFile, pkgs, emacs-ci }:
with builtins;
with pkgs.lib;
with (import ./nix/lib { inherit pkgs; });
let
  # Example:
  ...
  emacsDerivation = assert (isString emacs || isAttrs emacs);
  ...
  customEmacsPackages = pkgs.emacsPackagesFor emacsDerivation;

  # Built-in checkers and test drivers
  checkers = import ./nix/checkers { inherit pkgs customEmacsPackages; };

  emacsWithLocalPackages = packages_:
    customEmacsPackages (epkgs: (forEach packages_ checkers.melpaBuild));
  ...
  isDhallProject = hasSuffix ".dhall" packageFile;
  ...
  forEachPackage = forEach (attrValues packages);

  # Generate an attr set from packages with a function applied on each value
  mapPackage = f: mapAttrs (name: package: f package) packages;
